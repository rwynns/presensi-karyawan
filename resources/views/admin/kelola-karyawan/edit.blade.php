...
@section('content')
    <div class="py-4">
        <div class="flex items-center space-x-2 mb-6">
            <a href="{{ route('admin.kelola-karyawan.show', $kelola_karyawan) }}"
                class="text-gray-500 hover:text-gray-700 transition-colors">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path>
                </svg>
            </a>
            <h1 class="font-bold text-2xl">Edit Karyawan</h1>
        </div>
    </div>
        ...
        <div class="bg-primary-600 px-6 py-4">
            <div class="flex items-center">
                <div class="w-10 h-10 bg-white/20 rounded-full flex items-center justify-center flex-shrink-0">
                    <span class="text-sm font-bold text-white font-inter">
                        {{ strtoupper(substr($kelola_karyawan->nama, 0, 1)) }}
                    </span>
                </div>
                <div class="ml-4">
                    <h2 class="text-white font-semibold font-inter">Edit Data Karyawan</h2>
                    <p class="text-primary-100 text-sm font-inter mt-1">{{ $kelola_karyawan->nama }} -
                        {{ $kelola_karyawan->email }}</p>
                </div>
            </div>
        </div>

        <form action="{{ route('admin.kelola-karyawan.update', $kelola_karyawan) }}" method="POST" id="karyawanForm"
            class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="lg:col-span-2">
                    ...
                    <input type="text" name="nama" id="nama"
                        ...
                        value="{{ old('nama', $kelola_karyawan->nama) }}" placeholder="Masukkan nama lengkap karyawan"
                        required>
                    @error('nama')
                ...
                <div>
                    ...
                    <input type="email" name="email" id="email"
                        ...
                        value="{{ old('email', $kelola_karyawan->email) }}" placeholder="contoh@email.com" required>
                    @error('email')
                        <p class="mt-1 text-sm text-red-600 font-inter">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="no_hp" id="no_hp"
                        ...
                        value="{{ old('no_hp', $kelola_karyawan->no_hp) }}" placeholder="08xxxxxxxxxx" required>
                    @error('no_hp')
                        <p class="mt-1 text-sm text-red-600 font-inter">{{ $message }}</p>
                    @enderror
                </div>
                    ...
                    <select name="jabatan_id" id="jabatan_id"
                        ...
                        @foreach ($jabatans as $jabatan)
                            <option value="{{ $jabatan->id }}"
                                {{ old('jabatan_id', $kelola_karyawan->jabatan_id) == $jabatan->id ? 'selected' : '' }}>
                    ...
                    <select name="lokasi_id" id="lokasi_id"
                        ...
                        @foreach ($lokasis as $lokasi)
                            <option value="{{ $lokasi->id }}"
                                {{ old('lokasi_id', $kelola_karyawan->lokasi_id) == $lokasi->id ? 'selected' : '' }}>
                ...
                <div class="lg:col-span-2">
                    ...
                    <textarea name="alamat" id="alamat" rows="3"
                        ...
                        placeholder="Masukkan alamat lengkap karyawan" required>{{ old('alamat', $kelola_karyawan->alamat) }}</textarea>
                    @error('alamat')
                        <p class="mt-1 text-sm text-red-600 font-inter">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="lg:col-span-2">
                    ...
                    <div class="flex items-center">
                        <input type="checkbox" name="is_active" id="is_active" value="1"
                            {{ old('is_active', $kelola_karyawan->is_active) ? 'checked' : '' }}
                    ...
                    <p class="mt-1 text-xs text-gray-500 font-inter">
                        Status aktif memungkinkan karyawan untuk melakukan login dan presensi
                    </p>
                </div>
                ...
                <div class="lg:col-span-2 bg-gray-50 p-4 rounded-lg">
                    <div class="flex items-start">
                        ...
                        <div class="ml-3">
                            <h4 class="text-sm font-medium text-gray-900 font-inter">Informasi Update</h4>
                            <div class="text-xs text-gray-600 font-inter mt-1 space-y-1">
                                <p>• Akun dibuat: {{ $kelola_karyawan->created_at->format('d F Y H:i') }}</p>
                                <p>• Terakhir diupdate: {{ $kelola_karyawan->updated_at->format('d F Y H:i') }}</p>
                                @if ($kelola_karyawan->email_verified_at)
                                    <p>• Email terverifikasi:
                                        {{ Carbon\Carbon::parse($kelola_karyawan->email_verified_at)->format('d F Y H:i') }}
                                    </p>
                                @else
                                    <p class="text-yellow-600">• Email belum terverifikasi</p>
                                @endif
                            </div>
                        </div>
                    </div>
